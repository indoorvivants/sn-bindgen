...
object BindgenRender:
  ...
  private def sideBySide(
    ...
    val c =
      s"""<div style = "width: 40%; text-align: left"><b>$filename</b><pre><code class = "hljs c">""" + cSource
        .replace("\n", "<br />") + "</pre></code></div>"
    val scala =
      """<div style = "overflow: scroll;"><b>Generated Scala</b><div style = "max-height:400px"><pre><code class = "hljs scala">""" + scalaSource + "</code></pre></div></div>"
    val foot = "</div>"

    val generatedC =
      generatedCSource.map { code =>
        """<div style = "overflow: scroll;"><b>Generated C</b><div style = "max-height:400px"><pre><code class = "hljs scala">""" + code + "</code></pre></div></div>"
      }

    head + c + """<div style = "width:55%">""" + scala + generatedC.getOrElse("") + "</div>" + foot
  end sideBySide
    ...
    scala.util.Try(
      os.proc(cmdC)
        .call(
          os.pwd,
          stdout =
            ProcessOutput.Readlines(str => generatedC.append(str + "<br>"))
        )
    )
