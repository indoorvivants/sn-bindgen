...
object predef:
  private[lib_test_typedefs] trait _BindgenEnumCUnsignedInt[T](using
      eq: T =:= CUnsignedInt
  ):
...
object aliases:
  ...
  object duckdb_type:
    ...
    extension (v: duckdb_type) inline def value: DUCKDB_TYPEs = v
end aliases
...
object structs:
  ...
  object DUCKDB_COLUMNs:
    given _tag: Tag[DUCKDB_COLUMNs] =
      Tag.materializeCStruct3Tag[duckdb_type, CString, Ptr[Byte]]
    def apply()(using Zone): Ptr[DUCKDB_COLUMNs] =
      scala.scalanative.unsafe.alloc[DUCKDB_COLUMNs](1)
    def apply(`type`: duckdb_type, name: CString, internal_data: Ptr[Byte])(
        using Zone
    ): Ptr[DUCKDB_COLUMNs] =
      val ____ptr = apply()
      (!____ptr).`type` = `type`
