...
object predef {
    private[lib_test_use_braces] trait _BindgenEnumCUnsignedInt[T](using eq: T =:= CUnsignedInt) {
  ...
  */
  ...
  object Test {
    ...
    extension (struct: Test) {
      ...
      def hello_=(value: Test.Struct0): Unit = !struct.at(0).asInstanceOf[Ptr[Test.Struct0]] = value
    }
  }
...
private[lib_test_use_braces] object extern_functions {
  ...
  import _root_.lib_test_use_braces.structs.*
  import _root_.lib_test_use_braces.unions.*
  private[lib_test_use_braces] def __sn_wrap_lib_test_use_braces_naughty(st : Ptr[Hello]): Unit = extern

  private[lib_test_use_braces] def __sn_wrap_lib_test_use_braces_run(i : CInt, h : Float, test : Ptr[HelloAlias], verify : Test): CUnsignedInt = extern
